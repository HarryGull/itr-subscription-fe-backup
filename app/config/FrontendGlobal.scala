...
object LoggingFilter extends FrontendLoggingFilter with MicroserviceFilterSupport {
  override def controllerNeedsLogging(controllerName: String): Boolean = ControllerConfig.paramsForController(controllerName).needsLogging
}
...
object AuditFilter extends FrontendAuditFilter with RunMode with AppName with MicroserviceFilterSupport {
  ...
  override def controllerNeedsAuditing(controllerName: String): Boolean = ControllerConfig.paramsForController(controllerName).needsAuditing
}
