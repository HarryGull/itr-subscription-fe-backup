...
class TAVCAuthSpec extends UnitSpec with WithFakeApplication {
  ...
  "Government Gateway Provider" should {
    "have an account type additional parameter set to individual" in {
      val ggw=new GovernmentGatewayProvider(MockConfig.startUrl,MockConfig.ggSignInUrl)
  ...
  "Government Gateway Provider" should {
    "have a continueURL constructed from its first constructor parameter, encoded" in {
      val ggw=new GovernmentGatewayProvider(MockConfig.startUrl,MockConfig.ggSignInUrl)
      ggw.continueURL shouldEqual URLEncoder.encode(MockConfig.startUrl, "UTF-8")
    }
  }
  ...
  "TAVCAnyRegime" should {
    "ensure user is authorised if PAYE account exists in session" in {
      AuthTestController.TAVCAnyRegime.isAuthorised(authorisedUserAccounts) shouldBe true
    }
  }

  "TAVCAnyRegime" should {
    "ensure user is authorised even if PAYE account does not exist in session" in {
      AuthTestController.TAVCAnyRegime.isAuthorised(nonAuthorisedUserAccounts) shouldBe true
    }
  }
  ...
  "Calling TAVCStrongCredentialPredicate with an auth context that has strong credentials and CL50 confidence" should {
    "result in page is visible" in {

      val predicate = new TAVCStrongCredentialPredicate(
        MockConfig.startUrl,
  ...
  "Calling TAVCStrongCredentialPredicate with an auth context that has weak credentials and CL50 confidence" should {
